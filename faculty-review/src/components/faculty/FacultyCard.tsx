...
export default function FacultyCard({ faculty, index = 0, compact = false }: FacultyCardProps) {
  ...
  // Build subtitle: department name, or school abbreviation (in parens), or fallback
  const subtitle = dept?.name
    || (school?.name ? school.name.match(/\(([^)]+)\)/)?.[1] : null)
    || faculty.designation
    || "Faculty";

  // Compute display rating: use overall_rating if present, else average imported metrics
  const displayRating = faculty.overall_rating > 0
